...
function Toolbox() {
	...
	this.selectedTool = null;
	 ...
	 toolbarItemClick = function() {
		//remove any existing borders
		 items = selectAll(".sideBarItem");
		for ( i = 0; i < items.length; i++) {
			items[i].style('border', '0');
		}

		 toolName = this.id().split("sideBarItem")[0];
	...
	//add a new tool icon to the html page
	 addToolIcon = function(icon, name) {
		 sideBarItem = createDiv("<img src='" + icon + "'></div>");
